...
const ProjectCard = ({
  ...
  return (
    <motion.div whileInView={{ opacity: 1, transform: "none" }} variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      <Tilt
        style={{ backgroundColor: "cyan", boxShadow: "0px 0px 10px 3px grey" }}
        options={{
          max: 45,
          scale: 1,
          speed: 450,
      ...
      >
        <div className="Box1 relative w-full h-[180px]">
          <img
            src={image}
            alt="project_image"
            style={{ height: 250, width: 600, borderRadius: "20px", marginBottom: "20px" }}
          />
        ...
        <div style={{ marginTop: "90px" }} className="content mt-5">
          <p className="mt-2 text-secondary text-[14px]" style={{ textAlign: "justify", color: "black", fontSize: "18px", fontWeight: "500" }}>
            {description}
          </p>
        </div>
  ...
  return (
    <>
      <motion.div whileInView={{ opacity: 1, transform: "none" }} variants={textVariant()}>
        ...
        <h2 style={{ color: "skyblue" }} className={`${styles.sectionHeadText}`}>Projects.</h2>
      </motion.div>
      ...
      <div className="project w-full flex">
        ...
        >
          <div> {/* ✅ Changed from <p> to <div> */}
            <ul>
              {list.map((item) => (
                <ProjectList key={item.id} title={item.title} active={selected === item.id} setSelected={setSelected} id={item.id} />
              ))}
            </ul>
          </div>
